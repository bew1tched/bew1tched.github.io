...
define(["exports", "shader", "framebuffer", "data", "glMatrix"], //
    function (exports, shader, framebuffer, data) {
        ...
        function drawLineBresenham(startX, startY, startZ, endX, endY, endZ, color, storeIntersectionForScanlineFill, edgeStartVertexIndex, edgeEndVertexIndex, edgeStartTextureCoord, edgeEndTextureCoord) {
            ...
            let dXSign = startX < endX ? 1 : -1;
            let dYSign = startY < endY ? 1 : -1;

            let err = dX - dY;

            let interpolationWeight = 0;
            let edgeLength = getVectorLength(endX - startX, endY - startY, endZ - startZ)


            while (true) {
                framebuffer.set(startX, startY, getZ(startX, startY), color);

                addIntersection(startX, startY, getZ(startX, startY), interpolationWeight, edgeStartVertexIndex, edgeEndVertexIndex, null, null)
                let err2 = 2 * err;
        ...
        function assembleIntersectionForScanline(vertices, polygon, color, textureCoord, polygonTextureCoord, texture) {
            ...
            for (let i = 0; i < scanlineIntersection.length; i++) {
                if (scanlineIntersection[i] === undefined) continue;
                if (scanlineIntersection[i].length === 1 || scanlineIntersection[i].length % 2) continue;
                ...
                if (scanlineIntersection[i].length === 3) {
                    drawLineBresenham(scanlineIntersection[i][0].x, i, 0, scanlineIntersection[i][1].x, i, 0, data.getColorByName("black"));
                    drawLineBresenham(scanlineIntersection[i][1].x, i, 0, scanlineIntersection[i][2].x, i, 0, data.getColorByName("black"));
                }
                if (scanlineIntersection[i].length === 4) {
                    for (let j = 0; j < 4; j++) {
                        drawLineBresenham(scanlineIntersection[i][j].x, i, 0, scanlineIntersection[i][j + 1].x, i, 0, data.getColorByName("black"));
                        j++;
                    }
                }
            ...
            //drawLineBresenham(currX, currY, currZ, nextX, nextY, nextZ, color, true, st, end, null, null);
